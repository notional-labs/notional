...
3b) Download & copy the last version `v.1.4.1` to the upgrades folder.
This guide shows how to download the binary. If you want to build the binary from the source, detailed instructions can be found in the [README](https://github.com/BitCannaGlobal/bcna/blob/main/README.md) of our GitHub.
...
Extract and verify that the version is:`1.4.1`
```
...
* If you want to sync from the scratch (from block 1) you should do:
    ...
    ```
* If you are going to sync using **(advanced users)** a snapshot file, StateSync or other you should point to the last version (v1.4.1)
    ```
    ln -s -T ${HOME}/.bcna/cosmovisor/genesis ${HOME}/.bcna/cosmovisor/upgrades/strangeBuddheads
    ```
...
Environment=UNSAFE_SKIP_BACKUP=false #this will create a backup before apply the new upgrade
...
Environment=UNSAFE_SKIP_BACKUP=true #if you want to skip automatic backup
Environment=DAEMON_LOG_BUFFER_SIZE=512
...
* Show Cosmovisor version: `cosmovisor run version`  Will be `v.1.3.1` before the upgrade and `v1.4.1` after the upgrade
```
