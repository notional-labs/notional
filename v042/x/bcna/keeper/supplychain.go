...
func (k Keeper) AppendSupplychain(
	...
	appendedValue := k.cdc.MustMarshalBinaryBare(&supplychain)
...
func (k Keeper) SetSupplychain(ctx sdk.Context, supplychain types.Supplychain) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.SupplychainKey))
	b := k.cdc.MustMarshalBinaryBare(&supplychain)
	store.Set(GetSupplychainIDBytes(supplychain.Id), b)
}

// GetSupplychain returns a supplychain from its id
func (k Keeper) GetSupplychain(ctx sdk.Context, id uint64) types.Supplychain {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.SupplychainKey))
	var supplychain types.Supplychain
	k.cdc.MustUnmarshalBinaryBare(store.Get(GetSupplychainIDBytes(id)), &supplychain)
	return supplychain
}

// HasSupplychain checks if the supplychain exists in the store
func (k Keeper) HasSupplychain(ctx sdk.Context, id uint64) bool {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.SupplychainKey))
	return store.Has(GetSupplychainIDBytes(id))
}

// GetSupplychainOwner returns the creator of the
func (k Keeper) GetSupplychainOwner(ctx sdk.Context, id uint64) string {
	return k.GetSupplychain(ctx, id).Creator
}
